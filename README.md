...
**Part One only (no TensorFlow yet):** the minimum install above (`numpy`, `matplotlib`) is enough to start. Install the full `requirements.txt` before Part Two DNN and RL notebooks.

On **Apple Silicon Macs**, `pip install -r requirements.txt` automatically adds `tensorflow-metal` via an environment marker — no extra step needed.
